...
const modules = [
  { name: 'AI feature builder', href: '/admin', status: 'Ready', description: 'Describe a capability and receive a reviewable implementation proposal.', action: 'Generate proposal' },
  { name: 'Website factory', href: '/website-factory', status: 'Ready', description: 'Describe a site and route generation through the best configured builder.', action: 'Start generation' },
  { name: 'Workflow builder', href: '/workflows', status: 'Ready', description: 'Turn a prompt into an Activepieces workflow draft with activation and repair controls.', action: 'Create workflow' },
  { name: 'Social OS', href: '/social', status: 'Review', description: 'Create compliant scheduled content with manual approval and channel health.', action: 'Create content' },
  { name: 'Course studio', href: '/courses', status: 'Ready', description: 'Plan cohort and masterclass curriculum from a niche prompt.', action: 'Draft course' },
  { name: 'YouTube OS', href: '/youtube-os', status: 'Review', description: 'Research-led faceless content plans, thumbnails, and publishing queues.', action: 'Plan episode' },
  { name: 'AI designer', href: '/designer', status: 'Ready', description: 'Generate brand and graphic design directions from prompts or references.', action: 'Open designer' },
  { name: 'Lead generation', href: '/leads', status: 'Review', description: 'Keep project lead pipelines isolated with compliant outreach and alerts.', action: 'Create campaign' },
]
...
export function AutomationControlPlane() {
  ...
  return <div className="space-y-8">
    ...
    <section className="grid gap-4 md:grid-cols-2">{modules.map((module) => <article key={module.name} className="rounded-2xl border border-border bg-card p-5"><div className="flex items-start justify-between gap-4"><div><h3 className="font-semibold">{module.name}</h3><p className="mt-2 text-sm text-muted-foreground">{module.description}</p></div><span className={`rounded-full px-2.5 py-1 text-xs ${module.status === 'Ready' ? 'bg-emerald-500/10 text-emerald-600' : 'bg-amber-500/10 text-amber-600'}`}>{module.status}</span></div><div className="mt-5 flex flex-wrap gap-2"><a href={module.href} className="inline-flex items-center gap-2 rounded-lg bg-primary px-3 py-2 text-xs font-medium text-primary-foreground"><Play className="size-3" />{module.action}</a><button type="button" onClick={() => setMessage(`${module.name} health check queued.`)} className="inline-flex items-center gap-2 rounded-lg border border-border px-3 py-2 text-xs font-medium"><RefreshCw className="size-3" />Check health</button>{(module.status === 'Review' || activeModule === module.name) && <button type="button" onClick={() => setMessage(`${module.name} repair plan queued for review.`)} className="inline-flex items-center gap-2 rounded-lg border border-amber-500/30 px-3 py-2 text-xs font-medium text-amber-600"><AlertTriangle className="size-3" />Master fix</button>}</div></article>)}</section>
